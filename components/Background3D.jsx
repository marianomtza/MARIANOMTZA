 ...
 */
function Background3D({ showStars = true }) {
  const containerRef = React.useRef(null);
  const canvasRef = React.useRef(null);
  const stateRef = React.useRef({ x: 0, y: 0, mouseX: 0, mouseY: 0 });
  const isWebGLRef = React.useRef(false);
  ...
  React.useEffect(() => {
    ...
    if (canvas && canvas.getContext) {
      try {
        ...
        if (gl) {
          ...
          initWebGL(gl, canvas, stateRef.current);
        }
      } catch (e) {
    ...
    if (!isWebGLRef.current) {
      initCSS3DParallax(container, stateRef.current);
    }
...
Object.assign(window, { Background3D });
